...
void MapChipField::ResetMapChipData() {
	// マップチップデータをリセット
	mapChipData_1.data.clear();
	mapChipData_1.data.resize(kNumBlockVirtical);
	for (std::vector<MapChipType>& mapChipDataLine : mapChipData_1.data) {
	...
	for (uint32_t i = 0; i < kNumBlockVirtical; ++i) {
		...
		for (uint32_t j = 0; j < kNumBlockHorizontal; ++j) {
			...
			if (mapChipTable.contains(word)) {
				mapChipData_1.data[i][j] = mapChipTable[word];
			}
		}
	}
}
...
MapChipType MapChipField::GetMapChipTypeByIndex(uint32_t xIndex, uint32_t yIndex) {
	...
	return mapChipData_1.data[yIndex][xIndex];
}
